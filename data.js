...
const PARTIES={
 ...
 lib:{name:"Liberal Democrat",col:"#e8a13c",ideal:{e:-.4,s:-1},
   govSeats:332,oppSeats:72,unity:70,trust:58,app:48,
 ...
 grn:{name:"Green",col:"#3a9c4f",aiOnly:true,ideal:{e:-1.5,s:-1.2},
   govSeats:329,oppSeats:4,unity:66,trust:44,app:47,
...
/* ---------- scenarios ---------- */
const SCENARIOS={
 real:{name:"Britain, As Found",phase:"auto",desc:"July 2026, exactly as it is: Labour mid-term with 404 seats and a poll lead lost to Reform; everyone else plotting from real opposition benches. No modifiers — just the country.",
   setup:S=>{}},
 sterling:{name:"…And A Sterling Crisis",phase:"auto",desc:"The real baseline — plus the wheels coming off the economy: inflation 9.8%, gilts on strike, the IMF's number saved in the Chancellor's phone.",
   setup:S=>{S.econ.infl=9.8;S.econ.trust=28;S.econ.rates=6.5;S.econ.debt=112;S.econ.g=-0.8;S.svc.nhsWait=7.6}},
 coldwind:{name:"…And A Cold Wind",phase:"auto",desc:"The real baseline — plus Moscow probing theNorth Sea cables and Defence funded like a regional museum. The map will not stay quiet.",
   setup:S=>{S.world.regions.russia.rel=-85;S.flags.baltic2soon=true;S.mil.cap=0;S.fiscal.def=1.9}},
 press:{name:"…And A Hostile Press",phase:"auto",desc:"The real baseline — plus every front page out for you personally. Earn the coverage or break the proprietors.",
   setup:S=>{S.media.outlets.forEach(o=>o.stance-=14)}},
 partywar:{name:"…And A Party At War",phase:"auto",desc:"The real baseline — plus your own MPs in open revolt before you've hung a picture. The letters are already going in.",
   setup:S=>{S.party.factions.forEach(f=>f.happy-=15)}},
 honeymoon:{name:"…And A Honeymoon",phase:"auto",desc:"The real baseline — plus you've just taken the leadership and the country is curious. A gentler on-ramp.",
   setup:S=>{S.pols.approval+=7;S.pols.capital+=10}},
};
...
const REGIONS={
 ...
 uk:{n:"United Kingdom",lx:-30,ly:-22,lon:-0.1,lat:51.5,rel:100,trade:10,bloc:"home",cap:"London",home:true},
 france:{n:"France",lx:-14,ly:8,lon:2.3,lat:48.9,rel:38,trade:7,bloc:"eu",cap:"Paris"},
 germany:{n:"Germany",lx:34,ly:-14,lon:13.4,lat:52.5,rel:42,trade:8,bloc:"eu",cap:"Berlin"},
 eunorth:{n:"Nordics & Baltics",lx:26,ly:-20,lon:18.1,lat:59.3,rel:55,trade:6,bloc:"eu",cap:"Stockholm"},
 eusouth:{n:"Southern Europe",lx:16,ly:10,lon:12.5,lat:41.9,rel:35,trade:6,bloc:"eu",cap:"Rome"},
 ukraine:{n:"Ukraine",lx:30,ly:2,lon:30.5,lat:50.4,rel:60,trade:2,bloc:"east",cap:"Kyiv"},
 russia:{n:"Russia",lon:37.6,lat:55.8,rel:-65,trade:1,bloc:"east",cap:"Moscow"},
 mideast:{n:"Middle East",lx:-32,ly:6,lon:46.7,lat:24.7,rel:15,trade:5,bloc:"south",cap:"Riyadh"},
 gulf:{n:"The Gulf",lx:30,ly:8,lon:54.4,lat:24.5,rel:30,trade:6,bloc:"south",cap:"Abu Dhabi"},
 ...
 eastasia:{n:"Japan & Korea",lx:6,ly:10,lon:139.7,lat:35.7,rel:50,trade:7,bloc:"west",cap:"Tokyo"},
...
/* ---------- PMQs topic vocabulary: phrasings rotate, incidents feed in ---------- */
const PMQ_PHR={
 infl:["the cost of living","food prices","mortgage pain","energy bills"],
 nhs:["NHS waiting lists","A&E corridors","the GP appointment lottery","NHS dentistry"],
 mig:["migration numbers","the Channel crossings","asylum hotel costs","the visa system"],
 crime:["knife crime","shoplifting gangs","police response times","prison overcrowding"],
 jobs:["jobs and growth","stagnant wages","business confidence","high-street closures"],
 sleaze:["standards in government","ministers' second jobs","the honours system","lobbying rules"],
 war:["the war","conditions for our troops","defence spending","the path out of the conflict"]};
const PMQ_STATIC_K={mig:"mig",crime:"crime",climate:"jobs",eu:"jobs",culture:"sleaze",nhs:"nhs",housing:"jobs",defence:"war",econ:"jobs"};

/* ---------- the interview room: quoted answers, real consequences ----------
   opts: [quote, {effects}, risk(-/+ variance), ideo] ---------- */
const QBANK=[
 {k:"mig",q:"Net migration is running at %MIG%,000 a year. Is that too high?",opts:[
  ["“No. Migrants staff our hospitals and pay our pensions. The problem is housing, not people.”",-2],
  ["“The number matters less than fairness — fast decisions, safe routes, and employers paying properly.”",-1],
  ["“It's higher than the country is comfortable with, and trust depends on bringing it down honestly.”",0],
  ["“Yes. We'd set an annual cap and stick to it, whatever the lobbyists say.”",1],
  ["“Far too high. Net zero migration until the backlog is cleared and the hotels are empty.”",2]]},
 {k:"mig",q:"Would you take Britain out of the Refugee Convention to stop the boats?",opts:[
  ["“Never. You don't fix a queue by burning the rulebook on human beings.”",-2],
  ["“No — we'd fix it with processing centres in France and proper safe routes.”",-1],
  ["“We'd reform how it's applied. If that proves impossible, nothing is off the table.”",0],
  ["“If that's what it takes to control our borders, yes.”",1],
  ["“Out of the Convention, out of the ECHR, deportation flights weekly. Watch the boats stop.”",2]]},
 {k:"econ",q:"Taxes are at a seventy-year high. What would you actually do about it?",opts:[
  ["“Tax wealth properly — the broadest shoulders have been on holiday for a decade.”",-2],
  ["“Hold income tax, fund services first, and cut for working people only when it's paid for.”",-1],
  ["“Grow the economy first. Tax cuts you borrow for aren't cuts, they're deferrals.”",0],
  ["“Cut the basic rate, cut National Insurance, and make the state live within its means.”",1],
  ["“Slash taxes and slash the quangos. The state is twice the size it has any right to be.”",2]]},
 {k:"econ",q:"Would you borrow more to invest, even if the markets wobble?",opts:[
  ["“Yes — austerity was a forty-billion-pound false economy. Build now.”",-2],
  ["“For genuine infrastructure with independent oversight, yes, carefully.”",-1],
  ["“Borrowing for investment can be right; borrowing for day-to-day spending never is.”",0],
  ["“No. The gilt market has given this country two warnings. There won't be a third.”",1],
  ["“Not a penny more. Balance the budget and let the private sector build.”",2]]},
 {k:"nhs",q:"Should the NHS stay free at the point of use, no matter what?",opts:[
  ["“Yes — and end every privatising contract inside it. The NHS is not a market.”",-2],
  ["“Yes. Free at the point of use is who we are. Reform the management, never the principle.”",-1],
  ["“Yes on principle — but pretending it needs no reform is how we got eight million on lists.”",0],
  ["“Free at the point of use, but use private capacity ruthlessly to clear the backlog.”",1],
  ["“We should be honest: a social-insurance model works across Europe. Britain deserves the debate.”",2]]},
 {k:"nhs",q:"A nurse with ten years' service uses a food bank. What do you say to her?",opts:[
  ["“That she's been robbed by design — and we'd restore pay in full, year one.”",-2],
  ["“That her pay rise shouldn't depend on a strike. Multi-year restoration, agreed independently.”",-1],
  ["“That I won't insult her with promises before I can show her the costings.”",0],
  ["“That the way to raise her pay is a growing economy, not a wage spiral.”",1],
  ["“That public pay follows productivity — and the NHS needs reform before it needs cheques.”",2]]},
 {k:"crime",q:"Half of burglaries go unattended. Whose side are you on, honestly?",opts:[
  ["“The public's — which means youth services and prevention, not slogans and longer queues for court.”",-2],
  ["“Victims'. Neighbourhood officers back on every beat, courts actually sitting.”",-1],
  ["“There's no left or right answer to a break-in. Police you can see, justice that arrives.”",0],
  ["“Victims'. More stop and search, longer sentences, and no apologising for either.”",1],
  ["“The law-abiding majority's. Three strikes, automatic custody, and prisons built to hold them.”",2]]},
 {k:"crime",q:"Is stop and search racist?",opts:[
  ["“As practised? The numbers speak for themselves. Communities deserve policing by consent.”",-2],
  ["“It can be misused — body-cams, scrutiny, and trust are the fix, not abolition.”",-1],
  ["“It's a tool. Used with intelligence it saves lives; used lazily it poisons trust.”",0],
  ["“No. Knives are racist — they kill young Black men disproportionately. Search saves them.”",1],
  ["“This question is why the streets aren't safe. Police should police, not audit themselves.”",2]]},
 {k:"climate",q:"Net zero by 2050 — keep it, speed it up, or scrap it?",opts:[
  ["“Speed it up. 2040, a green jobs army, and make the polluters write the cheques.”",-2],
  ["“Keep it and actually deliver it — insulation, grid, nuclear. It's an economic plan, not a sermon.”",-1],
  ["“Keep the destination, be honest about the road. No targets that load costs onto the poorest.”",0],
  ["“Keep it only where it doesn't bankrupt households. Energy security first.”",1],
  ["“Scrap it. Drill the North Sea, build the gas plants, and let bills fall.”",2]]},
 {k:"eu",q:"Closer to Europe, or further away?",opts:[
  ["“Back into the single market this parliament — the experiment failed, say it plainly.”",-2],
  ["“Closer where it's win-win: food checks, youth mobility, defence. No rejoining rows.”",-1],
  ["“Wherever the national interest points, deal by deal. Theology is for seminars.”",0],
  ["“We're out. Diverge where it helps business and guard the borders of that decision.”",1],
  ["“Further. Scrap retained EU law, and any 'reset' that smells like re-entry is dead on arrival.”",2]]},
 {k:"culture",q:"Is Britain a racist country?",opts:[
  ["“Structurally, yes in places — and pretending otherwise insults the people living it.”",-2],
  ["“It has racism in it, like every country — and more decency than its loudest critics allow.”",-1],
  ["“It's one of the most tolerant countries on earth with real problems it shouldn't deny.”",0],
  ["“No. And the constant talking-down of this country is itself a national problem.”",1],
  ["“No — and the people who say it is are running the most successful grievance industry in Europe.”",2]]},
 {k:"culture",q:"Should schools teach children to be proud of British history?",opts:[
  ["“They should teach the truth — empire included — and let pride take care of itself.”",-2],
  ["“Teach all of it, the glory and the shame. Confident countries don't need a censored past.”",-1],
  ["“Yes — proud and honest aren't opposites. Trafalgar and the slave ports are both ours.”",0],
  ["“Yes. A country that teaches its children to be ashamed of itself doesn't last long.”",1],
  ["“Proudly and unapologetically. The curriculum reads like an apology written by a committee.”",2]]},
 {k:"housing",q:"Your party conference is in a town where the average house costs eleven times the average wage. What's the fix?",opts:[
  ["“Council houses — a hundred thousand a year — and rent controls while we build them.”",-2],
  ["“Build one and a half million homes and take the planning fights personally.”",-1],
  ["“Build more of everything, everywhere it makes sense — and reform the land market driving it.”",0],
  ["“Free up the green belt's scrubland, cut planning red tape, let builders build.”",1],
  ["“Cut immigration and the demand problem halves itself. Then build.”",2]]},
 {k:"defence",q:"Three per cent of GDP on defence — yes or no?",opts:[
  ["“No. The biggest threats are climate and poverty, and missiles don't fix either.”",-2],
  ["“Two and a half, spent competently, beats three wasted on procurement disasters.”",-1],
  ["“Yes, on a clear path — the world has gotten darker and pretending otherwise isn't strategy.”",0],
  ["“Yes, and quickly. Deterrence is cheaper than war.”",1],
  ["“Three by 2028, conscription debate on the table, and rebuild the Navy to rule the approaches.”",2]]},
 {k:null,q:"Why do you actually want this job?",opts:[
  ["“Because the people I grew up with keep paying for other people's mistakes, and I'm done watching.”",-1],
  ["“Because the country's problems are fixable and almost nobody in this trade talks like they are.”",0],
  ["“Because somebody has to run the place who's actually run something.”",1],
  ["“Power. Anyone who gives you a prettier answer is lying — the question is what you use it for.”",0],
  ["“Because my country called and you don't ask questions of that.”",1]]},
 {k:null,q:"Tell us something you've changed your mind about.",opts:[
  ["“I used to think the system just needed better managers. It needs rewiring.”",-1],
  ["“Tuition fees. I was wrong, the costings were wrong, and pretending otherwise was worse.”",0],
  ["“I used to think you could please the press. You can only out-work it.”",0],
  ["“I thought markets fixed everything. Then I watched a town die in five years.”",-1],
  ["“I used to believe in gentle persuasion. Westminster taught me to count votes instead.”",1]]},
];
...
const REAL_POLS={
 ...
 res:[["Ben Habib",34,62,48,58,"Foreign Sec."],["Esther McVey",35,58,55,52,"Home Sec."],["Nick Timothy",33,70,60,40,"Chancellor"],["Susan Hall",30,52,58,44,"Home Sec."],["Maria Bowtell",27,48,66,38,"Education Sec."],["Rev. Marcus Bell",31,58,66,56,"Education Sec."],["Col. Edmund Hartwell-Price",30,60,70,48,"Defence Sec."],["Dr. Cassandra Vane",29,68,52,44,"Health Sec."],["Peggy O'Mara",28,54,68,42,"Energy Sec."],["Hugh Considine",27,57,60,38,"Chief Whip"]],
};
...
const REAL_LEADERS={lab:"Keir Starmer",con:"Kemi Badenoch",lib:"Ed Davey",ref:"Nigel Farage",grn:"Zack Polanski",res:"Rupert Lowe",snp:"John Swinney"};
...
const GEN_POL=[
 ["a united party",null,S=>{S.party.factions.forEach(f=>f.happy+=10)}],
 ["a fractious party",null,S=>{S.party.factions.forEach(f=>f.happy-=12)}],
 ["a leader's honeymoon",null,S=>{S.pols.approval+=6;S.pols.capital+=8}],
 ["a leadership under siege",null,S=>{S.party.factions.forEach(f=>f.happy-=8);S.pols.sleaze+=8}],
 ["a flush war chest",null,S=>{S.pols.capital+=15;if(S.opp)S.opp.warchest+=5}],
 ["a broke machine",null,S=>{S.pols.capital-=12;if(S.opp)S.opp.warchest=1}],
 ["a defector wave brewing",null,S=>{S.flags.byCurse=true}]];
const GEN_WORLD=[
 ["calm seas",S=>{}],
...
const ISSUES={
 ...
 econ:{n:"Tax & spend",lo:"Invest, tax wealth",hi:"Cut taxes, shrink the state",pub:-0.1},
};
...
/* =====================================================================
   THE INCIDENT ENGINE — procedural month-to-month situations.
   Families x subjects x places x angles => >10,000 distinct decisions,
   layered under the hand-written set pieces.
   ===================================================================== */
const GE_PLACES=["Sunderland","Stoke-on-Trent","Wrexham","Dudley","Blackpool","Grimsby","Bradford","Middlesbrough","Swindon","Luton","Crawley","Carlisle","Ipswich","Mansfield","Rotherham","Bolton","Telford","Basildon","Hartlepool","Wigan","Doncaster","Worcester","Lincoln","Burnley","Gloucester","Norwich","Plymouth","Derby","Southampton","Peterborough","Oldham","Walsall","Stockport","Reading","Northampton","Slough","Bournemouth","Hull","Preston","Chesterfield","Dundee","Aberdeen","Paisley","Swansea","Newport","Bangor","Ballymena","Derry"];
const GE_EMPLOYERS=["Albion Steelworks","Crestline Motors","Polaris Foods","Brightway Logistics","Hartwell Ceramics","NorthGate Data","Pennine Textiles","Solent Marine","Kestrel Aerospace","Marchbank Brewery","Vantage Pharma","Caldon Glass"];
const GE_DEPTS=[["an NHS trust","Health Sec.","nhs"],["a prison","Home Sec.","crime"],["an academy chain","Education Sec.","schools"],["a rail franchise","Energy Sec.","housing"],["a water company","Energy Sec.","energy"],["a county council","Chancellor","housing"],["a border processing centre","Home Sec.","mig"],["a magistrates' court backlog","Home Sec.","crime"],["an army barracks","Defence Sec.","energy"],["a flood defence scheme","Energy Sec.","housing"],["a children's services department","Education Sec.","schools"],["an ambulance service","Health Sec.","nhs"],["a job centre network","Chancellor","housing"],["a social housing block","Chancellor","housing"]];
const GE_FAILS=["has gone bust overnight","is in special measures after a damning inspection","has declared a critical incident","is threatening indefinite strike action","has been hit by a catastrophic IT failure","is at the centre of a safety scandal"];
const GE_LOCALS=[["the last major factory","is closing with 1,800 jobs to go"],["the flood defences","failed again last night, two streets under water"],["the bypass campaign","has blockaded the high street for a sixth day"],["the football club","is in administration and the town is in mourning"],["the central library network","faces closure in budget cuts"],["the asylum accommodation","has sparked a tense standoff outside the town hall"],["the wind farm proposal","has split the town in half"],["the high street","has lost its bank, post office and pharmacy in one quarter"]];
const GE_PETS=["scrapping the TV licence","a four-day school week","banning phones for under-16s","rejoining the EU single market","bringing back national service","free dental care for all","a wealth tax on the super-rich","halving MPs' pay","St George's Day as a bank holiday","banning bottom trawling","votes for prisoners","scrapping smart motorways"];
const GE_INTL=[["summoned your ambassador over a stray remark","frosty"],["detained a British hiker at the border","tense"],["slapped surprise tariffs on Scotch whisky","costly"],["hacked a county council, allegedly","hostile"],["requested extradition of a London-based dissident","delicate"],["banned British beef on a technicality","petty"]];
const GE_STORMS=[["has your old expense claims from a decade ago","tedious but sticky"],["says your spouse's firm won a council contract","thin but loud"],["has WhatsApps of your team mocking a marginal seat","genuinely bad"],["claims you privately briefed against your own policy","deniable, just"],["has photos of you at a donor's villa","a bad look in a cost-of-living crisis"]];
const GE_CELEB=[["A national-treasure actor","called your government 'a tribute act to cruelty' on live TV"],["A retired England striker","is fundraising against your flagship policy"],["A bestselling children's author","has pulled out of a No.10 reception in protest"],["The country's biggest podcaster","spent ninety minutes monstering you to 4 million listeners"],["A chart-topping band","told their stadium crowd to vote against you"]];

const GE_FAMILIES=[
 {id:"dept",et:"Department crisis",phase:"government",
  mk:(S,pick,sev)=>{const dep=pick(GE_DEPTS),pl=pick(GE_PLACES),fl=pick(GE_FAILS);
   const min=S.cabinet.find(c=>c.role===dep[1])||S.cabinet[0];
   return{t:dep[0].replace(/^(an?|the) /,(m)=>m[0].toUpperCase()+m.slice(1))+" in "+pl+" "+fl.split(" ")[0]==="x"?"":"Crisis in "+pl,
    t2:"Crisis in "+pl,
    b:`${dep[0][0].toUpperCase()+dep[0].slice(1)} in ${pl} ${fl}. ${min.name} is on the radio in an hour; the local MP is already outside your office.`,
    opts:[
     {l:"Emergency intervention — money and a taskforce",s:"Grip it, own it",ideo:{e:-.5,s:0},eff:{app:1.2*sev,econ:{spendBump:.12*sev}}},
     {l:"Send "+min.name+" to take the heat",s:"That's what they're for",eff:{app:-.4*sev,unity:-1,media:1}},
     {l:"Independent review, report in spring",s:"The long grass, again",eff:{app:-.8*sev,capital:2}},
     {l:"Let it fail and blame the management",s:"Markets have consequences",ideo:{e:1.2,s:.3},eff:{app:-1.5*sev,trustM:2*sev}}]}}},
 {id:"local",et:"Local flashpoint",
  mk:(S,pick,sev)=>{const pl=pick(GE_PLACES),lc=pick(GE_LOCALS),emp=pick(GE_EMPLOYERS);
   return{t2:pl+": "+lc[0]+" crisis",
    b:`In ${pl}, ${lc[0]} ${lc[1].replace("1,800 jobs","1,800 jobs at "+emp)}. Cameras are there. Your diary says you're 200 miles away.`,
    opts:[
     {l:"Go there today",s:"Show up. It matters more than the diary",eff:{app:1.1*sev,capital:-3}},
     {l:"Announce a support package from London",s:"Substance without the photo",eff:{app:.5*sev,econ:{spendBump:.08*sev}}},
     {l:"A sympathetic statement",s:"Words are cheaper",eff:{app:-.6*sev}},
     {l:"Privately: not every town can be saved",s:"Never say it aloud",ideo:{e:1,s:.5},eff:{capital:3,app:-1*sev,media:-1}}]}}},
 {id:"petition",et:"The petition",
  mk:(S,pick,sev)=>{const p=pick(GE_PETS),n=120+Math.floor(sev*380);
   return{t2:"A petition lands: "+p,
    b:`${n},000 signatures for ${p}. Parliament must consider a debate; your MPs are split between mockery and panic.`,
    opts:[
     {l:"Adopt it — make it policy",s:"Ride the wave",ideo:{e:0,s:0},eff:{poll:.7*sev,unity:-2,promise:"Petition pledge: "+p}},
     {l:"Grant the debate, promise nothing",s:"Respect, cheaply",eff:{app:.3}},
     {l:"Dismiss it as clicktivism",s:"The base nods; the internet doesn't",eff:{media:-1.5,app:-.5*sev,unity:1}}]}}},
 {id:"intl",et:"Diplomatic incident",
  mk:(S,pick,sev)=>{const ks=Object.keys(REGIONS).filter(k=>k!=="uk"&&k!=="southatl");const rk=ks[Math.floor(S.rng()*ks.length)];
   const m=REGIONS[rk],inc=pick(GE_INTL);
   return{t2:m.n+" turns "+inc[1],
    b:`${m.cap} has ${inc[0]}. The Foreign Office wants calm; the tabloids want gunboats; the truth wants ten minutes of your attention.`,
    opts:[
     {l:"Call their leader personally",s:"De-escalate at the top",eff:{rel:{[rk]:6},standing:1,capital:-2}},
     {l:"Match them, measure for measure",s:"Reciprocity is a language",eff:{rel:{[rk]:-7},app:.6*sev,standing:-1}},
     {l:"Let officials handle it",s:"Not everything is yours",eff:{capital:2,rel:{[rk]:-2}}}]}}},
 {id:"storm",et:"Media storm",
  mk:(S,pick,sev)=>{const st=pick(GE_STORMS);const o=S.media.outlets[Math.floor(S.rng()*S.media.outlets.length)];
   return{t2:o.n+" comes for you",
    b:`${o.n} ${st[0]}. Verdict of your team: ${st[1]}. They want a comment by six.`,
    opts:[
     {l:"Full rebuttal, receipts out",s:"Kill it tonight or feed it for a week",eff:{media:1.5,app:.4*sev,capital:-3}},
     {l:"One line: 'no comment'",s:"Starve it",eff:{app:-.5*sev,sleaze:1.5*sev}},
     {l:"Lawyer up",s:"Chilling, in every sense",eff:{media:-2.5,sleaze:-1,app:-.4}},
     {l:"Self-deprecating post about it",s:"Risky charm",eff:{media:1*sev,app:.3}}]}}},
 {id:"celeb",et:"Celebrity row",
  mk:(S,pick,sev)=>{const c=pick(GE_CELEB);
   return{t2:"A famous enemy",
    b:`${c[0]} ${c[1]}. Four million likes and climbing. Your press team has drafted three responses and hates all of them.`,
    opts:[
     {l:"Invite them in for tea, publicly",s:"Disarm with grace",eff:{app:.8*sev,media:1}},
     {l:"'I don't take policy advice from celebrities'",s:"Base food",eff:{unity:1.5,media:-1,app:-.4*sev}},
     {l:"Ignore it entirely",s:"Famine beats fire",eff:{capital:2}}]}}},
 {id:"faction",et:"Backbench demand",
  mk:(S,pick,sev)=>{const f=S.party.factions[Math.floor(S.rng()*S.party.factions.length)];
   const iss=Object.keys(ISSUES)[Math.floor(S.rng()*8)];const I=ISSUES[iss];
   return{t2:f.name+" draw a line",
    b:`${f.leader} arrives with ${Math.round(8+sev*14)} colleagues and one demand: move on ${I.n.toLowerCase()} — their way — or 'consequences'. The lobby already knows.`,
    opts:[
     {l:"Give them most of it",s:"Peace has a price",eff:{unity:3,faction:{[f.name]:8},poll:-.4*sev}},
     {l:"Face them down",s:"Authority is use-it-or-lose-it",eff:{unity:-2.5*sev,faction:{[f.name]:-7},capital:4,app:.4}},
     {l:"Offer a review and a junior job",s:"The classic half-loaf",eff:{faction:{[f.name]:3},capital:-2}}]}}},
 {id:"oppgift",et:"The government stumbles",phase:"opposition",
  mk:(S,pick,sev)=>{const dep=pick(GE_DEPTS),pl=pick(GE_PLACES),fl=pick(GE_FAILS);
   return{t2:"Open goal in "+pl,
    b:`${dep[0][0].toUpperCase()+dep[0].slice(1)} in ${pl} ${fl} — on the government's watch. ${S.opp&&S.opp.gov.pm?S.opp.gov.pm:"The PM"} is refusing interviews. The goal is open.`,
    opts:[
     {l:"Go to "+pl+" and stand in front of it",s:"Pictures beat press releases",eff:{poll:.8*sev,gov:{app:-1*sev},capital:-3}},
     {l:"Urgent question in the House",s:"Procedure as theatre",eff:{gov:{app:-.7*sev},poll:.4,capital:-2}},
