...
(async () => {
  try {
    ...
    assert($("#bt-back"), "room overlay has a back-to-corridors button");
    assert($("#tab-hub .room"), "corridors stay rendered under the room");
    click($("#bt-back"), "back to the corridors");
    assert(!$("#tab-campaign").classList.contains("on"), "room overlay closed");
    assert($$("#tab-hub .room").length >= 12, "all rooms on the floorplan");
    click($('#gametabs button[data-t="world"]'), "situation room");
    assert(/WAR PLANNING/i.test($("#tab-world").textContent), "war planning panel visible");
    assert($("#wartarget") && $$("#wartarget option").length >= 10, "war target list populated");
    click($('#gametabs button[data-t="campaign"]'), "campaign again");
    click($('#gametabs button[data-t="world"]'), "situation for war planning");
    assert(/WAR PLANNING/i.test($("#tab-world").textContent), "war planning panel visible");
    assert($("#wartarget") && $$("#wartarget option").length >= 10, "war target list populated");
    click($('#gametabs button[data-t="campaign"]'), "back to campaign");
